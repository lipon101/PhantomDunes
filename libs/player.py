...
class Player:
    ...
    def __init__(self, terrain):
        ...
        #  Average Power (Watts) = âˆš 4.9 x body mass (kg)  x âˆš jump-reach score (m) x 9.81
    ...
    def loadSounds(self):
        ...
        self.sounds["damage"] = SoundSeries("assets/sounds/player/damage", 16, 0.6)
        self.sounds["evillaugh"] = Sound("assets/sounds/player/evillaugh.ogg", 1.0)
    ...
    def loop(self):
        # print(self.upSpeed)
        # self.spotLight()
        self.attackers = set() # Clear attackers each frame

        if getattr(self, "health", 100) <= 0:
            # Death triggered: play evil laugh ONCE and collapse
            if not hasattr(self, '_death_sound_played'):
                self._death_sound_played = True
                self.sounds["evillaugh"].play()
            
            if self.alive:
                self.alive = False
            
            if getattr(self, '_force_death_screen', False):
                self.health = -100 # Keep at -100 for extreme blood effect
            else:
                self.health = 0

            # Realistic collapse sequence - speed up the fall
            collapse_speed = 5.0 * self.sScale / self.tScale
            if self.height > 0.3 * self.sScale:
                self.height = max(self.height - collapse_speed, 0.3 * self.sScale)
