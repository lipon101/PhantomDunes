...
def _terminal_boot_sequence():
    if os.name == 'nt':
        os.system('color')
    
    # ANSI escape codes for modern terminal green and cyan
    green = '\033[92m'
    cyan = '\033[96m'
    reset = '\033[0m'
    
    print(f"\n{cyan}SYSTEM BOOT :: P H A N T O M   D U N E S{reset}")
    print("——————————————————————————————————————————")
    time.sleep(0.2)
    
    steps = [
        (" » World Generation", f"[{green} OK {reset}]", 0.3),
        (" » Entities Loaded", f"[{green} OK {reset}]", 0.4),
        (" » Survival Mode", f"[{cyan} ON {reset}]", 0.2)
    ]
    
    for task, status, delay in steps:
        sys.stdout.write(f"{task:<21}")
        sys.stdout.flush()
        time.sleep(delay)
        print(status)
...
if not hasattr(sys, '_boot_done'):
    sys._boot_done = True
    _terminal_boot_sequence()
...
try:
    ...
    glutCreateWindow(b"Phantom Dunes")
    # set premium taskbar icon on Windows
    if os.name == "nt":
        icon_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "phantom.ico"))
        if os.path.exists(icon_path):
            hwnd = ctypes.windll.user32.GetActiveWindow()
            if hwnd:
                icon = ctypes.windll.user32.LoadImageW(None, icon_path, 1, 0, 0, 0x00000010)
                if icon:
    ...
    _global_progress = 0.0
    def _draw_splash(add_prog=0.0):
        global _global_progress
        _global_progress += add_prog
        p = min(1.0, _global_progress)
        
        t = time.time()

        glClearColor(0.015, 0.015, 0.018, 1.0) # Sleek dark immersive premium background     
        ...
        if not hasattr(builtins, "splash_fnt"):
            from libs.font import Font
            builtins.splash_fnt = Font("assets/font/PHANTOM.png", "assets/font/PHANTOM.fnt") 

        def drawText_splash(text, y, color=(1, 1, 1, 1), scale=1.0):
            w = glutGet(GLUT_WINDOW_WIDTH)
            h = glutGet(GLUT_WINDOW_HEIGHT)
            wh = w / float(h) if h > 0 else 720.0 / 500.0
            
            # calculate width of text
            text_width = 0.0
            for char in text:
                if char in builtins.splash_fnt.data:
                    text_width += builtins.splash_fnt.data[char]['xadvance'] / float(builtins.splash_fnt.width)
            
            default_scale = 1.0 * scale
            # center x
            x = -(text_width * default_scale) / 2.0
            
            glMatrixMode(GL_PROJECTION)
            glPushMatrix()
            glLoadIdentity()
            # scaling for text to fit screen space coordinates: 
            # The screen goes from -1 to 1 basically when we don't set ortho.
            # but default font scale was 0.2
            font_scale = 0.2 * scale
            x = -(text_width * font_scale) / 2.0
            
            glTranslate(x, y, 0)
            glScalef(font_scale, font_scale * wh, 1.0)
            glColor4f(*color)
            builtins.splash_fnt.draw(text)
            glPopMatrix()
            glMatrixMode(GL_MODELVIEW)

        builtins.drawText_splash = drawText_splash

        # Deep minimalist backdrop
        glDisable(GL_TEXTURE_2D)

        # Sun Icon (Centered and Atmospheric)
        glPushMatrix()
        # Shift the entire group left by 0.085 to perfectly balance the two pyramids
        glTranslate(-0.085, -0.15, 0.0) # Lowered for breathing room
        glScalef(2.2, 2.2, 1.0)
        
        # Draw Sun
        glColor4f(0.85, 0.35, 0.05, 0.6) # Sleek modern transparency
        glBegin(GL_POLYGON)
        for i in range(30): # Smoother circle
            angle = i * (math.pi * 2 / 30)
            glVertex2f(0.0 + math.cos(angle)*0.1, 0.0 + math.sin(angle)*0.1*(720/500))
        glEnd()

        # Pyramid Icons (Sharp & Imposing)
        glBegin(GL_TRIANGLES)
        # Main Pyramid
        glColor4f(0.65, 0.45, 0.15, 1.0)
        glVertex2f(-0.18, -0.15)
        glVertex2f(0.18, -0.15)
        glVertex2f(0.0, 0.18)

        # Secondary Pyramid (Darker shadow side vibe)
        glColor4f(0.45, 0.25, 0.05, 0.95)
        ...
        # Premium modern Typography (Large, Perfectly Centered)       
        # PHANTOM DUNES text
        drawText_splash("PHANTOM DUNES", 0.68, (0.95, 0.95, 0.95, 1.0), 3.8)
        # SURVIVAL text underneath accurately
        drawText_splash("S U R V I V A L", 0.48, (0.9, 0.1, 0.1, 1.0), 1.5) # Bold clean red

        # Loading readout (Sleek corner styling)
        # Animating dots
        dots = int((t * 4) % 4)
        loading_text = "INITIALIZING" + "." * dots
        # We can center the loading dots at the bottom
        drawText_splash(loading_text, -0.78, (0.7, 0.7, 0.7, 0.9), 0.28)
        # We don't bother rendering percent text if it's offset, but we could 
        # drawText_splash(f"{int(p*100):02d} %", 0.6, -0.78, (0.9, 0.9, 0.9, 1.0), 0.28)

        # Modern Ultrathin Cinematic Loading Bar
        glDisable(GL_TEXTURE_2D)
        bar_y = -0.85
        bar_h = 0.004 # Razor thin
        bar_w = 0.7   # Wide wingspan

        # Bar background track
        glColor4f(0.1, 0.1, 0.1, 0.9)
        glBegin(GL_QUADS)
        glVertex2f(-bar_w, bar_y)
        glVertex2f(bar_w, bar_y)
        glVertex2f(bar_w, bar_y - bar_h)
        glVertex2f(-bar_w, bar_y - bar_h)
        glEnd()

        # Premium Pulsing Red Bar fill
        pulse = (math.sin(t * 6.0) + 1.0) * 0.2 + 0.8
        glColor4f(0.9 * pulse, 0.15 * pulse, 0.1 * pulse, 1.0)
        fill_w = -bar_w + (bar_w * 2 * p)
        glBegin(GL_QUADS)
        glVertex2f(-bar_w, bar_y)
        glVertex2f(fill_w, bar_y)
        glVertex2f(fill_w, bar_y - bar_h)
        glVertex2f(-bar_w, bar_y - bar_h)
        glEnd()
    ...
    builtins.update_loading_progress = _draw_splash
...
from libs.misc import drawOnScreen, drawRect, drawText
...
def draw():
    ...
    light_position = (2000, 4000, 2000)
    glLightfv(GL_LIGHT0, GL_POSITION, light_position)
    glLightfv(GL_LIGHT0, GL_DIFFUSE, (0.8, 0.7, \
        0.5, 1.0)) # Warmer sunlight
    ...
    if game.firstFrame:
        ...
        for en in game.enemies:
            en.lastAttackTime = pytime.time() + 3.0
        game.saveGame()
...
def pressedKey(key, xx, yy):
    if not rec.playing:
        ...
        if key == "\r" or key == "\n" or key == chr(13) or key == chr(10) or key == " ":
            if getattr(game.player, "gameOver", False):
                game.loadGame()
                # reset game over flag if not restored automatically
                game.player.gameOver = False
                game.player.health = 100.0
                game.player.alive = True
                if hasattr(game.player, '_death_sound_played'):
                    del game.player._death_sound_played
                if hasattr(game.player, '_force_death_screen'):
                    del game.player._force_death_screen
                return
