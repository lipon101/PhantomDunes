...
class Enemy:
    ...
    def __init__(self, player, tex, dyingSounds, pos=(0, 0, 0)):
        ...
        self.attackStrength = 12
        self.sphereR = (0.5 + 0.2)
        self.dead = False
        self.attacking = False
        self.attackCooldown = 0.9
        self.lastAttackTime = time() + 3.0
        self.attackRangeScale = 1.15
        self.closeInSpeed = 1.2
        self.attackCooldown = 0.9
        self.lastAttackTime = time() + 3.0
        # self.health = 100
        ...
        self.texture = tex
        self.animation = getPhantomAnimations()
    ...
    def attack(self):
        ...
        if distanceToPlayer < self.smellDist:
            ...
            desiredDist = (self.sphereR + self.player.sphereR) * self.attackRangeScale
            if distanceToPlayer > desiredDist:
                self.pos += vecToPlayer.normalize() * (self.speed * self.closeInSpeed)
            # start freaking out the player

            if distanceToPlayer < (self.sphereR + self.player.sphereR) * 0.75:
            ...
            elif distanceToPlayer <  (self.sphereR + self.player.sphereR)* 1.2:
            ...
            # don't attack
            if distanceToPlayer > (self.sphereR + self.player.sphereR) * self.attackRangeScale:
                self.attacking = False
            ...
            # attack for real don't just move your hands
                ...
                # Register as an active attacker
                if hasattr(self.player, 'attackers'):
                    self.player.attackers.add(self)

                now = time()
                
                # Instant death check if multiple villains attacking simultaneously
                total_ammo = self.player.gun.loadedBullets + self.player.gun.ammo
                is_fleeing = self.player.forward or self.player.backward or self.player.left or self.player.right or self.player.jump or self.player.running
                
                if (total_ammo <= 0 and not is_fleeing) or len(getattr(self.player, 'attackers', [])) > 1:
                    if self.player.alive:
                        self.player.health = -100
                        self.player.camera.cam_pos.y = self.player.terrain.heightPlus(self.player.getPos().tuple(), 0.3 * self.player.sScale)
                        self.player._force_death_screen = True
                        self.player.sounds["damage"].play()
                
                if now - self.lastAttackTime >= self.attackCooldown:
                    self.lastAttackTime = now
                    self.impactStart = now

                    if self.player.alive:
                        self.player.health -= self.attackStrength
                        self.player.sounds["damage"].play()
                        
                        # light knockback for impact feel (horizontal only)
                        knock_dir = (self.player.getPos() - self.pos).normalize()
                        knock_dir.y = 0 # Prevent the player from being knocked into the air
                        self.player.camera.cam_pos += knock_dir * (0.015 * self.sScale)
    ...
    def collisionWithOtherEnemy(self, enemy):
        ...
        # Smart swarm collision: allow them to get closer instead of repelling infinitely
        if vec2enemy.abs() < 1.1 * self.sphereR:
