...
def loadPackOfPak(path, step=1):
    ...
    prog_step = 0.4 / max(1, len(pakFiles))
    for i in pakFiles:
        t = time()
        clips[i.replace(".pak", "")] = loadPak(os.path.join(path, i), step=step)
        # print(time() - t)
        if hasattr(builtins, "update_loading_progress"):
            builtins.update_loading_progress(prog_step)
    return clips
...
# phantom and gun animation global variables
phantomAnimations = loadPackOfPak("assets/obj/enemy/phantom/lowPoly/", step=3)
gunAnimations = loadPackOfPak("assets/obj/player/weapon/", step=2)

# def loadGameAnimations():
#     global phantomAnimations, gunAnimations
#     phantomAnimations =
#     gunAnimations = loadPackOfPak("assets/obj/player/weapon/", step=1)


def getPhantomAnimations():
    temp = {}
    for i in phantomAnimations.keys():
        if i in ["walk", "attack", "walkAttack"]:
            tmp = True
        else:
            tmp = False
        temp[i] = Animation(phantomAnimations[i], step=1, speed=0.5, loop=tmp, randomIndex=tmp)
    return temp
