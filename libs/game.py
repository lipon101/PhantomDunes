...
class Game:
    ...
    for i in range(10):
        # Prevent enemies from spawning near the starting player location at (32, 32)
        while True:
            spawn_pos = terrain.getRandomPlace()
            if ((spawn_pos[0] - 32)**2 + (spawn_pos[2] - 32)**2)**0.5 > 15:
                break
        enemies.append(Enemy(player, enemyTexture, enemyDyingSound, spawn_pos))
    for i in range(10):
    ...
    def loadGame(self):
        ...
        # Fix: Absolute timestamps in save states cause instant-death attacks on restore
        # Dynamically grant 3 seconds of invulnerability upon respawn
        from time import time
        for enemy in self.enemies:
            enemy.lastAttackTime = time() + 3.0
    ...
    def cheatCodes(self):
        ...
        if lastEightChars[8 - 5:] == "reset":
            self.player.jumpPower = 2.4 * self.player.sScale / self.player.tScale
            self.player.gravity = 9.8 * self.player.sScale / self.player.tScale ** 2
            self.player.gun.loadedBullets = 6
            self.player.gun.ammo = self.player.gun.maxAmmo
            self.player.walkSpeed = 3.6 * self.player.sScale / self.player.tScale
            self.player.runSpeed = 5.5 * self.player.sScale / self.player.tScale
    ...
    def loop(self):
        ...
        if getattr(self.player, "gameOver", False):
            pass # Game strictly ended, freeze enemy logic
        else:
            for i in self.enemies:
                i.loop(self)
        ...
        if getattr(self.player, "gameOver", False):
            # Draw cinematic death overlay at the end so it renders on top
            ...
            # Dark cinematic vignette/veil (Red tinted for realism)
            glColor4f(0.15, 0.0, 0.0, 0.85) # darker, more opaque red background for contrast
            glBegin(GL_QUADS)
            glVertex2f(-1, -1)
            glVertex2f(1, -1)
            glVertex2f(1, 1)
            ...
            glEnable(GL_TEXTURE_2D) # RE-ENABLE TEXTURE SO LETTERS AREN'T BOXES!

            from libs.misc import drawText
            wh = glutGet(GLUT_WINDOW_WIDTH) / float(glutGet(GLUT_WINDOW_HEIGHT))
            
            import builtins
            if hasattr(builtins, "drawText_splash") and hasattr(builtins, "splash_fnt"):
                # Use the premium responsive text from the main game splash logic
                # Bright crisp blood red for "YOU DIED" against the new dark background
                builtins.drawText_splash("Y O U   D I E D", 0.15, (0.9, 0.1, 0.1, 1.0), 3.5)
                # Clean silver text for the stats
                kills_text = "S U R V I V A L   T I M E   O V E R   / /   K I L L S : {}".format(self.player.kills)
                builtins.drawText_splash(kills_text, -0.22, (0.85, 0.85, 0.85, 1.0), 0.75)
                # Pulsing or distinct instruction
                builtins.drawText_splash("> PRESS ENTER TO REBOOT <", -0.45, (0.6, 0.6, 0.6, 1.0), 0.55)
            else:
                # Fallback to older text
                drawText("Y O U   D I E D", -0.52, 0.20, wh, (0.9, 0.1, 0.1, 1.0), 1.5, projection=True)
                kills_text = "SURVIVAL TIME OVER // KILLS: {}".format(self.player.kills)
                drawText(kills_text, -0.55, -0.1, wh, (0.85, 0.85, 0.85, 1.0), 0.5, projection=True)
                drawText("> PRESS ENTER TO REBOOT <", -0.50, -0.75, wh, (0.6, 0.6, 0.6, 1.0), 0.45, projection=True)
    ...
    def waterFog(self):
        ...
        if self.player.camera.cam_pos.y > 0.3:
            ...
            fogColour = [0.8, 0.7, 0.5, 0.6]  # Sandstorm / Desert feel
